...
\tableofcontents
\abstract{
	Traditional feature engineering has been largely superseded by deep learning in many pattern recognition tasks in computer vision.
 However, we invent a new generalized feature engineering approach superior over deep learning in all aspects, accuracy, robustness, efficiency, explainability, maintainability. What one need for the new approach are only CPUs on one personal computer, zero GPUs, even for ImageNet. The core differences from traditional feature engineerrinig are: 1. a more appropriate geometric understanding for image pattern recognition; 2. a creation of a new programming language called Husky. The new geometric understanding shows the inherent flaw of deep learning and gives superior features. Husky is a new height in the evolution of programming language. It merges good designs from languages in different domains with a brand new paradigm we invented called Ascension, which greatly boosts programming efficiency for feature engineering, even making impossible possible. It comes with a debugging and visualization system tailored specifically for feature engineering, with no need to setup breakpoint, computation expandable as a timeless tree, from high level feature composition down to low level loops. Experiments show that it's as accurate as typical neural networks in MNIST and ImageNet, with inference 20 times faster for MNIST and 100 times faster for ImageNet. The approach is applicable for most tasks in computer vision. The programming language is even applicable for other AI domains, which will be the focus of future research.
}
